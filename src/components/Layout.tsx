  ...
  return (
    <div className="min-h-screen flex flex-col w-full overflow-x-hidden relative print:block print:min-h-0 print:overflow-visible">
      <header className={`fixed top-0 w-full z-50 transition-all duration-300 px-6 md:px-8 py-4 md:py-5 flex justify-between items-center print:hidden ${headerBg}`}>
        {/* Logo */}
        <Link to="/" className="text-lg md:text-xl tracking-wide text-brand-dark font-semibold">
          Shristi's Innovations
        </Link>

        {/* Desktop Nav (hidden on mobile) */}
        <nav className={`hidden md:flex gap-8 items-center text-sm font-medium tracking-wide transition-colors duration-300 ${scrolled || !isHome ? 'text-brand-dark' : 'text-white drop-shadow-md'}`}>
      ...
      <footer className="bg-brand-dark text-white py-16 pb-[100px] md:pb-16 px-8 flex justify-center flex-col items-center print:hidden">
        <h2 className="text-2xl mb-6">Shristi Srivastava</h2>
        <p
          className="mb-6 text-sm md:text-base cursor-pointer hover:text-brand-lime transition-colors border-b border-transparent hover:border-brand-lime"
